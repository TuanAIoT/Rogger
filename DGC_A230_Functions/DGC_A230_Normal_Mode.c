	...
	switch(Machine_State)
	{
		...
		case MACHINE_STATE_NORMAL_OPENING:
		{
			if (CFG_parameter.Sensing_Edge_COS1_Config != 0x00)
			{
				...
				Pre_flash_Time = 0x00;
				if(CFG_parameter.Number_Of_Motors == 0x02)
					__Do_Moving_Gate_After_COS_Action( CLOSE_MOVING_DIRECTION, 0x02);
				else
					__Do_Moving_Gate_After_COS_Action( CLOSE_MOVING_DIRECTION, 0x01);
		...
		case MACHINE_STATE_NORMAL_CLOSING:
		{
			if (CFG_parameter.Sensing_Edge_COS1_Config != 0x00)
			{
				if ((CFG_parameter.Sensing_Edge_COS1_Config == 0x03) || (CFG_parameter.Sensing_Edge_COS1_Config == 0x04))
				{
					...
					Pre_flash_Time = 0x00;
					if(CFG_parameter.Number_Of_Motors == 0x02)
						__Do_Moving_Gate_After_COS_Action( OPEN_MOVING_DIRECTION, 0x02);
					else
						__Do_Moving_Gate_After_COS_Action( OPEN_MOVING_DIRECTION, 0x01);
				}
			}
		} break;
