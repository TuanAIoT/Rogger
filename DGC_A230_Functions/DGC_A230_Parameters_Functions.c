...
//code [21]
uint16_t Automatic_Close_Flag;
uint16_t Automatic_Close_Count;
//code [21]
extern uint8_t Mask;
...
const char	CFG_Parameters_Max_Value[128]= {0x99, 0x01, 0x15, 0x01, 0x04, 0x10, 0x01, 0x01, 0x02, 0x99,
											...
											0x02, 0x99, 0x02, 0x04, 0x04, 0x03, 0x08, 0x08, 0x02, 0x99,
...
void Calculate_CFG_Parameters (void)
{
	...
	CFG_parameter.M1_Time_Increase = Val2;										// MOTOR 1 operating time increase	= [22] * 100
	Val3 = convert_BCD_to_binary(CFG_Parameters_Value[11]);
	CFG_parameter.M1_Maneuver_Time				= (Val1 + Val2)*(100 - Val3);	// Motor 1 Main maneuver time 		= ([17]+[22])*(100-[11])

	Val1 = convert_BCD_to_binary(CFG_Parameters_Value[19]);
	Val2 = convert_BCD_to_binary(CFG_Parameters_Value[23]);
	CFG_parameter.M2_Time_Increase = Val2;										// MOTOR 2 operating time increase	= [23] * 100
	Val3 = convert_BCD_to_binary(CFG_Parameters_Value[12]);
	CFG_parameter.M2_Maneuver_Time				= (Val1 + Val2)*(100 - Val3);	// Motor 2 Main maneuver time 		= ([19]+[23])*(100-[12])

	Val1 = convert_BCD_to_binary(CFG_Parameters_Value[17]);
	Val2 = convert_BCD_to_binary(CFG_Parameters_Value[22]);
	Val3 = convert_BCD_to_binary(CFG_Parameters_Value[11]);
	CFG_parameter.M1_Deceleration_Time			= (Val1+Val2)*Val3;				// Motor 1 Deceleration time 		= ([17]+[22])*[11]

	Val1 = convert_BCD_to_binary(CFG_Parameters_Value[19]);
	Val2 = convert_BCD_to_binary(CFG_Parameters_Value[23]);
	Val3 = convert_BCD_to_binary(CFG_Parameters_Value[12]);
	CFG_parameter.M2_Deceleration_Time			= (Val1+Val2)*Val3;				// Motor 2 Deceleration time 		= ([19]+[23])*[12]

	Val3 = convert_BCD_to_binary(CFG_Parameters_Value[11]);
	CFG_parameter.M1_Deceleration_Space = Val3;

	Val3 = convert_BCD_to_binary(CFG_Parameters_Value[12]);
	CFG_parameter.M2_Deceleration_Space = Val3;

	CFG_parameter.Motor_Finish_Time				= CFG_Parameters_Value[41]*100;	// Motor finish time				= [41]*100;
	...
	if (Val1 <= 91)
		CFG_parameter.Automatic_Closing_Time 	= Val1*100;						// 21: Setting automatic closing time = [21]*100	0-91 second
	else
		CFG_parameter.Automatic_Closing_Time 	= (Val1-90)*6000;				// 21: Setting automatic closing time = [21]*60*100	2-9  minute
	...
	{
	case 0x00: 	CFG_parameter.Motor_Torque_Boots	= CFG_parameter.Motor_Torque_Maneuver; break;
	case 0x01: 	CFG_parameter.Motor_Torque_Boots	= TORQUE_LEVEL_1; break;
	case 0x02: 	CFG_parameter.Motor_Torque_Boots	= TORQUE_LEVEL_2; break;
	case 0x03: 	CFG_parameter.Motor_Torque_Boots	= TORQUE_LEVEL_3; break;
	case 0x04: 	CFG_parameter.Motor_Torque_Boots	= TORQUE_LEVEL_4; break;
	...
	case 0x07: 	CFG_parameter.Motor_Torque_Boots	= TORQUE_LEVEL_7; break;
	case 0x08: 	CFG_parameter.Motor_Torque_Boots	= TORQUE_LEVEL_8; break;
	default: 	CFG_parameter.Motor_Torque_Boots	= CFG_parameter.Motor_Torque_Maneuver; break;
	}
	...
	while ((working_done == 0) && (Key_Tick_Count < 1000))
	{
		...
		if ((!DOWN_KEY) && (Machine_State == MACHINE_STATE_STOP))
		{
			...
			if (((CFG_Paramenters_Index < 100) || (CFG_Paramenters_Index > 119)) && (CFG_Paramenters_Index != 90))
			{
				...
				//code [A2]
				// chinh tham so A2 tu 01 xuong 99
				if ((CFG_Paramenters_Index == 2) && (Parameter_Value[1] == '1') && ((CFG_Parameters_Value[4] == 0x02) || (CFG_Parameters_Value[4] == 0x01)))
				{
					Parameter_Value[1] = '0';
				}
				//code [A2] end
		...
		//code [A2] [A4]
		//neu A4 = 01, 02 va A2 = 00 thi A2 = 01
		if ((CFG_Paramenters_Index == 2) && (CFG_Parameters_Value[2] == 0x00) && ((CFG_Parameters_Value[4] == 0x02) || (CFG_Parameters_Value[4] == 0x01)))
		{
				Parameter_Value[1] = '1';
		}
		//code [A2] [A4] end
