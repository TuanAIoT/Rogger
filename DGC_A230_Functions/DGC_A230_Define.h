...
#define SENSING_EDGE_STATUS_INACTIVE	0x00
#define SENSING_EDGE_STATUS_ACTIVE		0x01

#define GATE_MOVING_DIRECTION_UNKNOW	0x00
#define GATE_MOVING_DIRECTION_OPEN		0x01
#define GATE_MOVING_DIRECTION_CLOSE		0x02

#define TORQUE_LEVEL_8					0x05DB	// 250 us
#define TORQUE_LEVEL_7					0x1D4B	// 1250 us
#define TORQUE_LEVEL_6					0x34BB	// 2250 us
#define TORQUE_LEVEL_5					0x4C2B	// 3250 us
#define TORQUE_LEVEL_4					0x639B	// 4250 us
#define TORQUE_LEVEL_3					0x7B0B	// 5250 us
#define TORQUE_LEVEL_2					0x927B	// 6250 us
#define TORQUE_LEVEL_1					0xA9EB	// 7250 us
...
#define MACHINE_STATE_COS_OPENING		0x05
#define MACHINE_STATE_COS_CLOSING		0x06
#define MACHINE_STATE_RECOVERY_OPENING	0x07
#define MACHINE_STATE_RECOVERY_CLOSING	0x08
...
#define GATE_OPEN_INDICATOR_MODE_0 		0x00
#define GATE_OPEN_INDICATOR_MODE_1		0x01
#define GATE_OPEN_INDICATOR_MODE_2 		0x02
#define GATE_OPEN_INDICATOR_MODE_3 		0x03

#define AUTO_CLOSE_GATE_COMMAND_OFF		0x00
#define AUTO_CLOSE_GATE_COMMAND_ON		0x01

#define STOP_TIMER_PERIOD_NUMBER		0x14
...
struct Device_Config_Parameters
	...
	uint16_t 	M1_Travel_Time;						// Motor 1 Main maneuver time 		= ([17]+[22])*(100-[11])
	uint16_t 	M2_Travel_Time;						// Motor 2 Main maneuver time 		= ([19]+[23])*(100-[12])
	...
	uint16_t    M1_Total_Space;
	uint16_t    M2_Total_Space;

	uint16_t    M1_Partial_Space;

	uint16_t	M1_Deceleration_Space;				// Motor 1 Deceleration				= [11]
	uint16_t	M2_Deceleration_Space;				// Motor 2 Deceleration				= [12]

	uint16_t	M1_Maneuver_Space;					// Motor 1 Deceleration				= [11]
	uint16_t	M2_Maneuver_Space;					// Motor 2 Deceleration				= [12]

	uint8_t 	Motor_Deceleration_Level;			// Motor Deceleration Level			= [41];
	...
	uint16_t 	Automatic_Closing_Time;				// 21: Setting automatic closing time = [21]*2 or [21]*60*2
	...
	uint16_t 	Reverse_Time_After_Edge_Sense;		// Reverse time after activation of sensing edge = [27]*100
	...
	uint16_t 	Lock_Activation_Lead_Time;			// Electric lock activation lead time 		= [28]*100
	uint16_t 	Lock_Activation_Time;				// Electric lock activation time 			= [29]*100
	uint16_t 	Lock_Release_Reverse_Impulse;		// Electric lock release reverse impulse	= [38]*100
	...
	uint16_t 	Braking_Time;						// 64: Set braking time = [64]*10
	uint16_t 	Braking_Force;						// 65: Set braking force
